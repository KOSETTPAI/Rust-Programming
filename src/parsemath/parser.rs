...
impl<'a> Parser<'a> {
    ...
    pub fn parse(&mut self) -> Result<Node, ParseError> {
        ...
        match ast {
            ...
            Ok(ast) => Ok(ast),
            Err(_) => Err(ParseError::UnableToParse("Unable to parse".to_string())),
        }
    }
}
...
impl<'a> Parser<'a> {
    ...
    fn generate_ast(&mut self, oper_prec: OperPrec) -> Result<Node, ParseError> {
        ...
        loop {
            let current_prec = self.current_token.get_oper_prec();
            
            // Break if we hit EOF or if the next operator's precedence isn't higher
            if self.current_token == Token::EOF || oper_prec >= current_prec {
                break;
            }
            
            left_expr = self.convert_token_to_node(left_expr)?;
        }
    ...
    fn convert_token_to_node(&mut self, left_expr: Node) -> Result<Node, ParseError> {
        match self.current_token {
            ...
            Token::And => {
                self.get_next_token()?;
                let right_expr = self.generate_ast(OperPrec::AndOr)?;  // Lower precedence
